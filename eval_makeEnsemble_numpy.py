    ...
    def run_evaluate(self, data_manager, model_save_path, result_save_path):
        ...
        with tf.device('/cpu:0'), session.as_default():
            ...
            while remain_batch > 0:
                ...
                prediction_total.append(prediction_batch)
            ...
            prediction_total = np.array(prediction_total).reshape([-1])


            np.save('./labelData.npy', label_total)
            np.save('./predictionData.npy', prediction_total)
            np.save('./matData.npy', mat_total)
            np.save('./degData.npy', deg_total)
            np.save('./brightData.npy', bright_total)
