...
class Evaluator:
    def __init__(self):
        ...
        """
        print '[!] START INITIALIZING EVALUATOR'
        with tf.device('/cpu:0'):
            ...
            self.images = self.all_towers[0]['images']
            self.att_maps = self.all_towers[0]['att_maps']
    ...
    def run_evaluate(self, data_manager, model_save_path, result_save_path):
        ...
        with tf.device('/cpu:0'), session.as_default():
            ...
            map_save_path=os.path.join(result_save_path, 'maps')
            ...
            while remain_batch > 0:
                ...
                prediction_batch,images,att_maps = session.run([self.output,self.images,self.att_maps], feed_dict={self.input: input_batch, self.label: label_batch})
                print np.shape(images)
                print np.shape(att_maps)

                for s in range(batch_size*config.IMAGE_FRAMES):
                    gray_image = np.reshape(images[s],[config.IMAGE_WIDTH,config.IMAGE_HEIGHT])
                    gray_att_maps = np.reshape(att_maps[s], [config.IMAGE_WIDTH, config.IMAGE_HEIGHT])

                    scipy.misc.imsave(os.path.join(map_save_path,str(s)+'_'+str(remain_batch)+'_ori_image.png'), gray_image)
                    scipy.misc.imsave(os.path.join(map_save_path, str(s) + '_' + str(remain_batch) + '_att_image.png'),
                                      gray_att_maps)
